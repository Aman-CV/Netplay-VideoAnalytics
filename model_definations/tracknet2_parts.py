...
class Down(nn.Module):
    ...
    def __init__(self, n, in_channels, out_channels, bn_first=True):
        ...
        self.maxpool_conv = nn.Sequential(
            nn.MaxPool2d(2),
            DoubleConv(in_channels, out_channels, bn_first=bn_first) if n == 2 else TripleConv(in_channels,
                                                                                               out_channels,
                                                                                               bn_first=bn_first)
        )
...
class Up(nn.Module):
    ...
    def __init__(self, n, in1_channels, in2_channels, out_channels, bilinear=True, mode='nearest', bn_first=True,
                 halve_channel=True):
        super().__init__()
        if not n in [2, 3]:
            raise ValueError('n must be 2 or 3')
        ...
        if bilinear:
            ...
            if halve_channel:
                ...
                if n == 2:
                    self.conv = DoubleConv(in1_channels // 2 + in2_channels, out_channels, mid_channels=out_channels,
                                           bn_first=bn_first)
                elif n == 3:
                    self.conv = TripleConv(in1_channels // 2 + in2_channels, out_channels, mid_channels=out_channels,
                                           bn_first=bn_first)
            else:
                self.up = nn.ConvTranspose2d(in1_channels, in1_channels, kernel_size=2, stride=2)
                if n == 2:
                    self.conv = DoubleConv(in1_channels + in2_channels, out_channels, mid_channels=out_channels,
                                           bn_first=bn_first)
                elif n == 3:
                    self.conv = TripleConv(in1_channels + in2_channels, out_channels, mid_channels=out_channels,
                                           bn_first=bn_first)
